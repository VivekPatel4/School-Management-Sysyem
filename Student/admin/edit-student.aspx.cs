...
namespace Student.admin
{
    public partial class edit_student : System.Web.UI.Page
    {
        ...
        private void LoadStudentData(int sid)
        {
            using (SqlConnection con = new SqlConnection(connect))
            {
                string q = "select stdid, RoomNo, FirstName, LastName, EmailId, Department, Gender, Dob, Phonenumber, photo, ParentName, Occupation, ParentMobileno, Country, State, District, Pincode, City, Address, Password from students  where id = @id";
                ...
                if (r.Read())
                {
                    ...
                    txtconfirmpassword.Text = r["Password"].ToString();
                }
                else
                {
                    ErrorPanel.Visible = true;
